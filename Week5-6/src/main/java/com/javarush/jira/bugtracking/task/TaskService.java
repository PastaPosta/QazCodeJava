...
public class TaskService {
    ...
    public void addTag(long taskId, String tag){
        ...
        if(tagTrim.length()<2 | tagTrim.length()>32){
            throw new IllegalArgumentException("Tag lenght 2<x<32");
        }

        Task task = handler.getRepository().getExisted(taskId);
        task.getTags().add(tagTrim);
    }
    ...
    public Duration getTimeInWork(Task task){
        LocalDateTime inProgress = getLatestActivityTimeByStatus(task, "in_progress");
        LocalDateTime review = getLatestActivityTimeByStatus(task, "ready_for_review");

        if(inProgress != null && review != null && review.isAfter(inProgress)){
            return Duration.between(inProgress, review);
        }
    ...
    public Duration getTimeInTesting(Task task){
        LocalDateTime review = getLatestActivityTimeByStatus(task, "ready_for_review");
        LocalDateTime done = getLatestActivityTimeByStatus(task, "done");

        if(done != null && review != null && done.isAfter(review)){
            return Duration.between(review, done);
        }
        return Duration.ZERO;
    }

    public LocalDateTime getLatestActivityTimeByStatus(Task task, String statusCode){
        if(task.getActivities() == null) return null;

        return task.getActivities().stream()
                .filter(t -> statusCode.equals(t.getStatusCode()))
                .map(Activity::getUpdated).filter(Objects::nonNull)
