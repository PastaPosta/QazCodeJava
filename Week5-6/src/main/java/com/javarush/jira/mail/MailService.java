...
public class MailService {
    ...
    // 1. Изменяем метод: теперь он подхватывает локаль из контекста запроса
    public String sendToUserWithParams(@NonNull String template, @NonNull User user, @NonNull Map<String, Object> params) {
        String emailTo = Objects.requireNonNull(user.getEmail());
        Map<String, Object> extParams = Util.mergeMap(params, Map.of("user", user));

        Locale locale = LocaleContextHolder.getLocale();
        return send(appConfig.isProd() ? emailTo : appProperties.getTestMail(), user.getFirstName(), template, extParams, locale);
    }
    public String send(String toEmail, String toName, String template, Map<String, Object> params, Locale locale) {
        log.debug("Send email to {}, {} with template {} and locale {}", toEmail, toName, template, locale);
        String result = OK;
        try {
    ...
    private String getContent(String template, Map<String, Object> params, Locale locale) {
        Locale notNullLocale = (locale != null) ? locale : Locale.ENGLISH;
